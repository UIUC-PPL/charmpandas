...
arrow::Status CreateUserTable(int num_rows, int num_procs) {
    ...
    for (int pe = 0; pe < num_procs; pe++) {
        ...
        // Create table
        auto table = arrow::Table::Make(schema, {user_ids_array, cities_array});
...
arrow::Status CreateAgesTable(int num_rows, int num_procs) {
    ...
    for (int pe = 0; pe < num_procs; pe++) {
        ...
        {
            int idx = indices[i];
            user_ids.push_back(num_rows * pe + idx);
        ...
        // Create table
        auto table = arrow::Table::Make(schema, {user_ids_array, ages_array});
