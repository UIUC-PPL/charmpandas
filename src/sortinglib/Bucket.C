...
template <class key>
void Bucket<key>::startBarrier(CProxy_Sorter<key> _sorter_proxy, CProxy_HSSMain<key> _main_proxy){
...
//      CkCallback(CkIndex_Sorter<key>::globalMinMax(NULL), sorter_proxy)); 
    ...
    CkCallback(CkIndex_HSSMain<key>::init_isum(NULL),main_proxy));
#endif
}
...
void Bucket<key>::Load(data_msg<key>* msg){
    ...
    if(recvd == nBuckets){
        ...
        this->contribute(sizeof(int), &tsum, CkReduction::sum_int,
                CkCallback(CkIndex_HSSMain<key>::intermediate_isum(NULL),main_proxy));
    }
...
void Bucket<key>::postMerging(){
    if (callBackSet)
        ...
        this->contribute(sizeof(int), &keysum, CkReduction::sum_int,
            CkCallback(CkIndex_HSSMain<key>::final_isum(NULL),main_proxy));
