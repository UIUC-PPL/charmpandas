...
void HistSorting(int input_elems_, key* dataIn_, 
    ...
    if(CkMyPe() == 0) {
        ...
        //#endif
        static CProxy_HSSMain<key> mainProxy = CProxy_HSSMain<key>::ckNew(CkNumPes(), probe_max, CkNumNodes(), CkMyPe());
        //mainProxy.DataReady();
    }
}
...
template <class key>
class HSSMain : public CBase_HSSMain<key> {
  private:
  ...
  public:
      HSSMain (int num_buckets_, int probe_max, int num_partitions_) {
          //number of buckets (data chares)
          ...
          CkStartQD(CkCallback(CkIndex_HSSMain<key>::DataReady(NULL), this->thisProxy));
          //DataReady(); 
      }
      //migration constructor
      HSSMain (CkMigrateMessage *m) { }
      //starts sorting
...
template <class key>
void HSSMain<key>::DataReady(CkReductionMsg *msg) {
    //!TODO: Could there be a data race here?
    CkPrintf("Data generated and ready at all processors\n"); 
...
template <>
void recursive_pup_impl<HSSMain<unsigned long>, 1>::operator()(HSSMain<unsigned long> *obj, PUP::er &p) {
    obj->parent_pup(p);
    obj->HSSMain::pup(p);
}
